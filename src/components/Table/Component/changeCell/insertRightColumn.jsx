...
const insertRightColumn = ({editorState, onChange}) => {
    ...
    for (let row = 0; row < tableShape.length; row++) {
        if (!('columnspan' in tableShape[row][col]) || tableShape[row][col].columnspan > 1) {
            if (col !== tableShape[row].length - 1) {
                if (!('columnspan' in tableShape[row][col+1]) || tableShape[row][col+1].columnspan > 1) {
                    console.log("Can't insert column because next have merge cell");
                    return editorState;
                }
            }
        }
    ...
    // Chèn cột mới vào bên phải của cột được chọn
    for (let row = 0; row < tableShape.length; row++) {
        tableShape[row].splice(col + 1, 0, { columnspan: 1, rowspan: 1 });
    }
    ...
    for (let row = 0; row < tableShape.length; row++) {
        const newBlock = new ContentBlock({
            ...
            data: Map({
                tableKey: tableKey,
                cellPosition: `${tableKey}-${row}-${col + 1}`
            })
        });
        newBlockMap = newBlockMap.set(newBlock.getKey(), newBlock);
    }
...
export default insertRightColumn;